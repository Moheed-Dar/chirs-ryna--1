  ...
  return (
    <>
      ...
      <style>{`
        ...
        .blog-detail-scroll::-webkit-scrollbar-thumb:hover {
      ...
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-9999 bg-[#172636]/95 flex items-center justify-center p-4 overflow-hidden"
        onClick={onClose}
      >
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          exit={{ opacity: 0, scale: 0.95 }}
          className="relative bg-[#1E3040] rounded-2xl border border-[#FFF7F0]/10 w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl"
          onClick={(e) => e.stopPropagation()}
        >
          {loading ? (
            <div className="flex-1 flex items-center justify-center p-10">
            ...
            <>
              ...
              <div
                className="blog-detail-scroll flex-1 min-h-0 overflow-y-auto overscroll-contain p-6 space-y-6"
                data-lenis-prevent
              >
                {/* ✅ Check both featuredImage and image for fallback */}
                {(blog.featuredImage?.url || blog.image) && (
                ...
                <div>
                  <h3 className="text-sm font-bold text-[#FFF7F0] uppercase tracking-wider mb-2">Content</h3>
                  {/* ✅ Rich Text HTML rendering with styling */}
                  <div 
                    className="text-sm text-[#FFF7F0]/70 leading-relaxed bg-[#1F2D3D] p-4 rounded-xl border border-[#FFF7F0]/5 overflow-hidden wrap-break-word
                               [&_p]:my-2 [&_div]:my-2 
                               [&_ul]:list-disc [&_ul]:pl-6 [&_ul]:my-2 
                ...
                {/* Bottom spacing so content footer ke bilkul upar tak na chipke */}
                <div className="h-2" />
              </div>
