...
export default function PropertyDetailView({
  ...
  // ============================================
  // ✅ SCROLL FIX: Lock body scroll while drawer is open
  // (Background page scroll nahi hogi, sirf drawer scroll hogi)
  // ============================================
  useEffect(() => {
  ...
  if (loading) {
    return (
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-9999 bg-black/60 overflow-hidden"
        onClick={onClose}
      >
        <motion.div
          initial={{ x: "100%" }}
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .property-detail-scroll::-webkit-scrollbar-track {
        ...
        .property-detail-scroll::-webkit-scrollbar-thumb:hover {
      ...
      >
        <motion.div
          ...
          exit={{ x: "100%" }}
          transition={{ type: "spring", damping: 30, stiffness: 300 }}
          className="absolute right-0 top-0 bottom-0 w-full max-w-2xl bg-[#081730] flex flex-col overflow-hidden shadow-2xl shadow-black/50"
          onClick={(e) => e.stopPropagation()}
        >
          {/* ===== HEADER ===== */}
          <div className="shrink-0 border-b border-white/6 bg-[#081730]">
          ...
          <div
            className="property-detail-scroll flex-1 min-h-0 overflow-y-auto overscroll-contain"
            data-lenis-prevent
          >
            {/* ---- IMAGE GALLERY ---- */}
            {images.length > 0 && (
