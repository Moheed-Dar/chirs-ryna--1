...
//   return (
//     <div className="bg-[#1F2D3D] border border-[#FFF7F0]/8 rounded-xl overflow-hidden focus-within:border-[#F2673A]/40 focus-within:ring-2 focus-within:ring-[#F2673A]/10 transition-all">
//       {/* Toolbar */}
...
//             e.target.value = "";
//           }}
//           className="bg-[#1F2D3D] border border-[#FFF7F0]/8 rounded-md px-2 py-1 text-xs text-[#FFF7F0]/70 outline-none mr-1 cursor-pointer hover:border-[#F2673A]/40"
//           defaultValue=""
//           title="Font Size"
//         >
...
export default function BlogsUpdateForm({ blog, onClose, onSuccess }) {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 z-9999 bg-[#172636]/95 flex items-center justify-center overflow-hidden" onClick={onClose}>
        <Loader2 size={32} className="animate-spin text-[#F2673A]/50" />
      </div>
    );
  }
  ...
  return (
    <>
      {/* ✅ SCROLL FIX: Custom scrollbar styles (orange theme) */}
      <style>{`
        .blog-update-scroll {
          scrollbar-width: thin;
          scrollbar-color: rgba(242, 103, 58, 0.35) transparent;
        }
        .blog-update-scroll::-webkit-scrollbar {
          width: 6px;
        }
        .blog-update-scroll::-webkit-scrollbar-track {
          background: transparent;
        }
        .blog-update-scroll::-webkit-scrollbar-thumb {
          background: rgba(242, 103, 58, 0.35);
          border-radius: 999px;
        }
        .blog-update-scroll::-webkit-scrollbar-thumb:hover {
          background: rgba(242, 103, 58, 0.6);
        }
      `}</style>
      ...
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-9999 bg-[#172636]/95 overflow-hidden"
        onClick={onClose}
      >
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: 10 }}
          className="relative z-10 flex flex-col h-full overflow-hidden bg-[#1F2D3D]"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header */}
          <div className="shrink-0 border-b border-[#FFF7F0]/6 bg-[#1F2D3D]">
            ...
            {error && (
              <div className="px-6 lg:px-8 pb-3">
                <div className="px-4 py-2.5 bg-[#D81B60]/10 border border-[#D81B60]/15 rounded-xl flex items-center gap-3">
                  <AlertCircle size={15} className="text-[#D81B60] shrink-0" />
                  <p className="text-[#D81B60] text-xs flex-1 wrap-break-word">{error}</p>
                  <button onClick={() => setError("")} className="p-0.5 hover:bg-[#D81B60]/20 rounded-lg">
                    <X size={13} className="text-[#D81B60]" />
                  </button>
                </div>
          ...
          {/* ===== SCROLLABLE CONTENT ===== */}
          {/* ✅ SCROLL FIX: min-h-0 + data-lenis-prevent added */}
          <div
            className="blog-update-scroll flex-1 min-h-0 overflow-y-auto overscroll-contain"
            data-lenis-prevent
          >
            <form
              id="blogUpdateForm"
            ...
            <div className="flex items-center justify-between px-6 lg:px-8 h-16 max-w-4xl mx-auto">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                ...
                <button
                  ...
                  form="blogUpdateForm"
                  disabled={saving}
                  className="flex items-center gap-2 px-7 py-2.5 bg-[#F2673A] hover:bg-[#db5a32] disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-xl transition-all shadow-lg shadow-[#F2673A]/20"
                >
                  {saving ? (
                    <><Loader2 size={16} className="animate-spin" /> Updating...</>
                  ) : (
