...
const validators = {
  ...
  currency: (value) => {
    if (!value) return null;
    // ✅ Case-insensitive check (usd → USD)
    if (!ALLOWED_CURRENCIES.includes(String(value).toUpperCase())) return `Currency must be one of: ${ALLOWED_CURRENCIES.join(', ')}`;
    return null;
  },
...
// ==========================================
// ✅ NORMALIZE ENUM FIELDS (after validation passes)
// ==========================================
...
const updatePropertyHandler = async (request, context, user) => {
  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      // ==========================================
      // ✅ NORMALIZE propertyType / currency / status
      // ==========================================
      normalizeEnumFields(updateData);
